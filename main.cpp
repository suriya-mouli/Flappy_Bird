...
void Bird(float Y)
{
    ...
    glColor3f(0.2, 0.4, 0.8); // Blue body
    Circle(-5, Y, 0.6, 0, 360);

    // Head
    glColor3f(0.0, 0.0, 0.0);
    Circle(-4.6, Y + 0.2, 0.4, 0, 360);
    glColor3f(0.2, 0.4, 0.8);
    Circle(-4.6, Y + 0.2, 0.37, 0, 360);

    // Eyes
    glColor3f(1.0, 1.0, 1.0); // White of eye
    Circle(-4.5, Y + 0.3, 0.15, 0, 360);
    glColor3f(0.0, 0.0, 0.0); // Pupil
    Circle(-4.5, Y + 0.3, 0.07, 0, 360);

    // Beak
    glColor3f(1.0, 0.8, 0.2); // Yellow beak
    glBegin(GL_POLYGON);
    glVertex2f(-4.3, Y + 0.2);
    glVertex2f(-4.1, Y + 0.25);
    glVertex2f(-4.3, Y + 0.3);
    glVertex2f(-4.3, Y + 0.25);
    glEnd();

    // Crest (head feathers)
    glColor3f(0.0, 0.0, 0.0);
    glBegin(GL_TRIANGLES);
    glVertex2f(-4.6, Y + 0.6);
    glVertex2f(-4.8, Y + 0.8);
    glVertex2f(-4.4, Y + 0.8);
    glEnd();
...
void PIPE1(int X)
{
    ...
    //Up - New decorative pipe design
    glColor3f(0.2, 0.5, 0.2); // Darker green base
    glBegin(GL_POLYGON);
    ...
    glBegin(GL_POLYGON);
    glVertex2f(PX[0] + X - 0.1, Y + P_G + 0.2);
    glVertex2f(PY[0] + X + 0.1, Y + P_G + 0.2);
    glVertex2f(PY[0] + X, Y + P_G);
    glVertex2f(PX[0] + X, Y + P_G);
    glEnd();

    // Pipe pattern - stripes
    glColor3f(0.3, 0.8, 0.3);
    for (float stripe = Y + P_G + 0.5; stripe < 50; stripe += 0.8) {
        glBegin(GL_POLYGON);
        glVertex2f(PX[0] + X, stripe);
        glVertex2f(PY[0] + X, stripe);
        glVertex2f(PY[0] + X, stripe + 0.3);
        glVertex2f(PX[0] + X, stripe + 0.3);
        glEnd();
    }

    //Down - New decorative pipe design
    glColor3f(0.2, 0.5, 0.2); // Darker green base
    glBegin(GL_POLYGON);
    glVertex2f(PX[0] + X, -50);
    glVertex2f(PY[0] + X, -50);
...
void PIPE2(int X)
{
    ...
    glColor3f(0.0, 0.7, 0.0);
    glBegin(GL_POLYGON);
    glVertex2f(PX[1] + X - 0.1, Y + P_G + 0.2);
    glVertex2f(PY[1] + X + 0.1, Y + P_G + 0.2);
    glVertex2f(PY[1] + X, Y + P_G);
    glVertex2f(PX[1] + X, Y + P_G);
    glEnd();
    ...
    for (float stripe = Y + P_G + 0.5; stripe < 50; stripe += 0.8) {
        glBegin(GL_POLYGON);
        glVertex2f(PX[1] + X, stripe);
        glVertex2f(PY[1] + X, stripe);
        glVertex2f(PY[1] + X, stripe + 0.3);
        glVertex2f(PX[1] + X, stripe + 0.3);
        glEnd();
    }
...
void PIPE3(int X)
{
    ...
    glColor3f(0.0, 0.7, 0.0);
    glBegin(GL_POLYGON);
    glVertex2f(PX[2] + X - 0.1, Y + P_G + 0.2);
    glVertex2f(PY[2] + X + 0.1, Y + P_G + 0.2);
    glVertex2f(PY[2] + X, Y + P_G);
    glVertex2f(PX[2] + X, Y + P_G);
    glEnd();
    ...
    for (float stripe = Y + P_G + 0.5; stripe < 50; stripe += 0.8) {
        glBegin(GL_POLYGON);
        glVertex2f(PX[2] + X, stripe);
        glVertex2f(PY[2] + X, stripe);
        glVertex2f(PY[2] + X, stripe + 0.3);
        glVertex2f(PX[2] + X, stripe + 0.3);
        glEnd();
    }
...
void PIPE4(int X)
{
    ...
    glColor3f(0.0, 0.7, 0.0);
    glBegin(GL_POLYGON);
    glVertex2f(PX[3] + X - 0.1, Y + P_G + 0.2);
    glVertex2f(PY[3] + X + 0.1, Y + P_G + 0.2);
    glVertex2f(PY[3] + X, Y + P_G);
    glVertex2f(PX[3] + X, Y + P_G);
    glEnd();
    ...
    for (float stripe = Y + P_G + 0.5; stripe < 50; stripe += 0.8) {
        glBegin(GL_POLYGON);
        glVertex2f(PX[3] + X, stripe);
        glVertex2f(PY[3] + X, stripe);
        glVertex2f(PY[3] + X, stripe + 0.3);
        glVertex2f(PX[3] + X, stripe + 0.3);
        glEnd();
    }
...
void GameCheck(int X)
{
	...
	for (int i = 0; i < 4; i++)
	{
		...
		if (-5 == PX[i] + X || -5 == PY[i] + X)
		{
			...
			{
				Score++;
                PlaySound(TEXT("C:\\Users\\hp\\Documents\\Project\\Flappy_Bird\\score.wav"), NULL, SND_ASYNC | SND_FILENAME);
			}
		}
	}
	if (L)
	{
        ...
        PlaySound(TEXT("C:\\Users\\hp\\Documents\\Project\\Flappy_Bird\\crash.wav"), NULL, SND_ASYNC | SND_FILENAME);
	}
	else
	{
		//cout << "CONTINUE" << endl;
...
void Jump(int btn, int state, int x, int y) {
	...
	if (btn == GLUT_LEFT_BUTTON)
	{
		...
		if (state == GLUT_DOWN && S != 3)
		{
			B_H = B_H + 2;
			Bird(B_H);
			 PlaySound(TEXT("C:\\Users\\hp\\Documents\\Project\\Flappy_Bird\\flap.wav"), NULL, SND_ASYNC | SND_FILENAME);
		}
	}
}
